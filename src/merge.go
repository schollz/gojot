...
func MergeEncrypted(mergedEncrypted string, pass string) string {
	begin := "-----BEGIN PGP SIGNATURE-----\n\n"
	end := "\n-----END PGP SIGNATURE-----"
	etext1 := begin + StrExtract(mergedEncrypted, "<<<<<<< HEAD", "=======\n", 1) + end
	etext2 := begin + StrExtract(mergedEncrypted, "=======\n", ">>>>>>>", 1) + end
	text1, err := DecryptString(etext1, pass)
	if err != nil {
		logger.Error("Problem decrypting merged string: \n'%s'", etext1)
		logger.Error(err.Error())
	}
	text2, err := DecryptString(etext2, pass)
	if err != nil {
		logger.Error("Problem decrypting merged string: \n'%s'", etext2)
		logger.Error(err.Error())
	}
...
func MergeText(text1, text2 string) string {
	seq1 := strings.Split(text1, "\n")
	seq2 := strings.Split(text2, "\n")
