...
# # Encyrpt using GPG
# cmd = 'gpg -q --no-use-agent --passphrase %s --symmetric --cipher-algo AES256 -o %s %s' % (
#     p, os.path.join(DATA_PATH, '.journal.txt.temp.gpg'), os.path.join(DATA_PATH, '.journal.txt.temp'))
# try:
...
#     clean_up()
#     sys.exit(1)
# os.system('mv %s %s' % (os.path.join(DATA_PATH, '.journal.txt.temp.gpg'),
#                         os.path.join(DATA_PATH, '.journal.txt')))
# clean_up()
#
# if is_connected():
#     print("Uploading...")
#     os.system("rsync -aq --update %s root@cowyo.com:.journal.txt" %
#               os.path.join(DATA_PATH, '.journal.txt'))
