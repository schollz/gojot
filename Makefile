...
prerelease:
	...
	env GOOS=windows GOARCH=amd64 go build ${LDFLAGS} -o sdees-${BUILDSHORT}.exe
	zip -j sdees_windows_amd64.zip sdees-${BUILDSHORT}.exe
	...
	cd src && sed -i -- 's/package main/package sdees/g' bindata.go
	env GOOS=windows GOARCH=amd64 go build ${LDFLAGS} -o sdees-${BUILDSHORT}.exe
	zip -j sdees_windows_amd64_withvim.zip sdees-${BUILDSHORT}.exe
	github-release upload \
			--user schollz \
			--repo sdees \
			--tag nightly \
